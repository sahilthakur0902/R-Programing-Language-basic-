...
df <- data.frame(gender, overallpercentage)


require(dplyr)
students <- bind_cols(students, df)
...
students_2 <- data.frame(name, age, majorsubject, pass, gender, overallpercentage)


students <- bind_rows(students, students_2)
...
students[students$name == 'Nitin', 1] <- 'Nitish'


#Q6.
# Rochelle now needs to show Name and OverallPercentage of Students who have failed 
# in any of the subjects. How to code that?

students[ students$pass == FALSE  , c('name', 'overallpercentage', 'pass') ]
...
robin <- data.frame(name = 'Robin', age = 34, 
                    majorsubject = 'Maths', pass= NA, gender = 'Male',
                    overallpercentage = NA)

robin

students_2 <- bind_rows(students, robin)
